					...
					$.validator,
					{
						...
						messages : {
							...
							rangelength : $.validator
									.format("字符长度必须在 {0} 到 {1} 之间"),
							range : $.validator.format("数字必须在 {0} 到 {1} 之间"),
							max : $.validator.format("不能大于 {0}"),
							min : $.validator.format("不能小于 {0}"),
							mobile : "请输入正确的手机号码",
							tel : "请输入有效的电话号码(区号-电话号-分机号)",
							category : "类别不能为空",
							mobileOrEmail : "手机号或邮箱地址格式有误",
							password : "密码只能是英文字母和数字",
							illegal : "含有非法字符"
						},
								...
								$(this.currentForm)
										.validateDelegate(
												":text, [type='password'], [type='file'], select, textarea, "
														...
														+ "[type='email'], [type='datetime'], [type='date'], [type='month'], [type='illegal'], "
						...
						methods : {
							...
							date : function(value, element) {
								...
								if(/^d+$/.test(value)){
									...
									_date = new Date();
									var _splits = value.split('-');
									_date.setFullYear(_splits[0], _splits[1], _splits[2]);
								}
								return this.optional(element)
										|| !/Invalid|NaN/.test(_date);
							},
							...
							illegal : function(value, element) {
								return this.optional(element)
										|| /^[^~!@#$%^&*()_+|{}:;=,.·……—【】‘’“”《》。、\uFF00-\uFFFF\/\\\[\]"'<>?`-]*$/
												.test(value);
							},
							category : function(value, element) {
								el = $(element);
								try {
									var elts = $('[plugincategoryid='
											+ el.attr('plugincategoryid') + ']');
									if (elts.length == 2) {
							...
							mobileOrEmail : function(value, element) {
								return this.optional(element)
										|| /^1[3|4|5|8][0-9]\d{8}$/.test(value)
										|| /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))$/i
												.test(value);
...
(function($) {
	...
	$.validator.addMethod("minute",function(value, element, param){
		var _regx = /^[\d]{4}-[\d]{2}-[\d]{2} [\d]{2}:[\d]{2}$/g;
		return this.optional(element) || (_regx).test('' + value);
	}, '日期格式错误');
	
	$.validator.addClassRules('minute', {
		minute : {
			minute : true
		}
	});
